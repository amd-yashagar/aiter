...
"""gfx942 packed-varlen FMHA (CDNA3).

K path matches production ASM / native fused FMHA: ``buffer_load_dword … lds``
into the padded ``lds_elem_offset`` layout (64x32 planes), SwizzleA on the
N-column reader. V uses the native register ``v_perm`` transpose followed by
``ds_write2_b32`` into padded 8x8 LDS tiles. Register-P GEMM2.

Tuned first for MLA skinny prefill (QK=192, V=128, bf16, packed THD). The
same kernel compiles other QK/V pairs that pass ``validate_fmha_gfx942_tiles``.

gfx942-only: 4B DMA-to-LDS (no 16B), no ``ds_read_tr``, no CDNA4 MFMA 32x32x16.
Production ``flash_attn_varlen_func`` stays on ASM until this path wins;
opt in with ``AITER_FMHA_FLYDSL_GFX942=1``.
"""
...
from aiter.ops.flydsl.kernels.fmha_gfx942.config import (
    BLOCK_M as DEFAULT_BLOCK_M,
    BLOCK_N as DEFAULT_BLOCK_N,
    BLOCK_SIZE,
    D_CHUNK,
...
def build_fmha_varlen_gfx942_module(
    num_heads,
    head_dim_qk,
    head_dim_v,
    causal,
    ...
    """Build a packed-THD varlen FMHA launcher for gfx942.

    ``block_m`` / ``block_n`` / ``waves_per_eu`` are fallback defaults from
    ``config.py`` unless the caller overrides them.
    """
    ...
    validate_fmha_gfx942_tiles(
        head_dim_qk, head_dim_v, block_m=block_m, block_n=block_n
    )
    ...
    k_allocator.ptr = LDS_K_TILE_SIZE * 2
    v_allocator = SmemAllocator(
        None,
        arch=gpu_arch,
        global_sym_name="fmha_varlen_gfx942_v_smem",
    )
    v_allocator.ptr = LDS_V_TILE_SIZE * 2

    @flyc.kernel(known_block_size=[BLOCK_SIZE, 1, 1])
    def fmha_varlen_gfx942_kernel(
        Q: fx.Tensor,
        K: fx.Tensor,
        V: fx.Tensor,
        ...
        def k_buf_base(buf_id):
            if const_expr(isinstance(buf_id, int)):
                return fx.Index(buf_id * K_PLANE_ELEMS)
            return buf_id * fx.Index(K_PLANE_ELEMS)

        def v_buf_base(buf_id):
            return fx.Index(buf_id * LDS_V_TILE_SIZE)
        ...
        wave_id_u = fx.Index(rocdl.readfirstlane(T.i32, fx.Int32(wave_id)))
        ...
        def coop_load_v_global(tile_start, n_half):
            ...
            row_in_wave = wave_id * fx.Index(8) + k_within * fx.Index(2)
        ...
        def coop_store_v_lds(vecs, n_half, buf_id=0):
            ...
            n_hdim = lane >> fx.Index(2)
            k_within = lane & fx.Index(3)
            k_in_half = wave_id * fx.Index(8) + k_within * fx.Index(2)
        ...
        for kv_block_start, inner_iter_args in range(
            ...
            for kv_sub in range_constexpr(1):
                ...
                for plane in range_constexpr(K_PLANES):
                    ...
                    if const_expr(plane + 1 < K_PLANES):
                        coop_dma_k_plane(
                            kv_start,
                            plane + 1,
                            (plane + 1) % K_STAGE_COUNT,
                        )
                        if const_expr(
                ...
                for r in range_constexpr(16):
                    ...
                    p_hi = fx.Float32(rocdl.exp2(T.f32, _raw(diff_hi)))
                ...
                if const_expr(D_CHUNKS <= PV_K_STEPS):
                ...
                if const_expr(CAUSAL):
                    ...
                    # four newly issued K DMAs remain outstanding.
                    _waitcnt_vm_n(4)
                    coop_store_v_lds(
                ...
                if const_expr(dtype_str == "bf16"):
                    ...
                    for pks in range_constexpr(PV_K_STEPS):
                        p_base = pks * 4
                        p_packs_lo.append(
                            bf16_trunc_pack_v4(p_vals_lo[p_base : p_base + 4])
                        )
                        p_packs_hi.append(
                            bf16_trunc_pack_v4(p_vals_hi[p_base : p_base + 4])
                        )
                else:
                    p_f16_lo = [
    ...
    launch_fmha_varlen_gfx942.compile_hints = dict(_fmha_compile_hints)
    return launch_fmha_varlen_gfx942
...
def _get_launcher(
    ...
    return build_fmha_varlen_gfx942_module(
        num_heads=num_heads,
...
def flash_attn_varlen_gfx942(
    ...
    *,
    ...
    """Packed-THD varlen FMHA on gfx942.

    ``kv_splits=None`` chooses a split-K factor that fills gfx942 CUs. Pass
    ``1`` to disable splitting. ``max_seqlen_k`` is used for that choice; the
    kernel still reads per-batch KV length from ``cu_seqlens_k``.
    """
    if q.dim() != 3 or k.dim() != 3 or v.dim() != 3:
        raise ValueError("q/k/v must be packed THD [total, H, D]")
    if not (q.is_cuda and k.is_cuda and v.is_cuda):
        raise ValueError("flash_attn_varlen_gfx942 requires CUDA/HIP tensors")
    if not (q.dtype == k.dtype == v.dtype):
        raise ValueError(f"q/k/v dtype must match: {q.dtype}/{k.dtype}/{v.dtype}")
    dtype_str = _DTYPE_TO_STR.get(q.dtype)
    if dtype_str is None:
        raise ValueError(
            f"flash_attn_varlen_gfx942 supports bf16/f16, got {q.dtype!r}"
        )
    ...
    validate_fmha_gfx942_tiles(dq, dv, block_m=block_m, block_n=block_n)
