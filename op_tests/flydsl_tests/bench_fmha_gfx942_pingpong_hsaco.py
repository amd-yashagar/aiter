...
"""Correctness-gated A/B bench for a reassembled gfx942 ping-pong FMHA HSACO.

The code object must be the non-causal, return-LSE, bf16 H=12/QK=192/V=128
eight-wave variant emitted by ``FLYDSL_DUMP_IR=1`` as
``fmha_pingpong_gfx942_kernel_0``. Launch is 512 threads, BLOCK_M=256.
"""
...
KERNEL = "fmha_pingpong_gfx942_kernel_0"
HEADS = 12
HEAD_DIM_QK = 192
HEAD_DIM_V = 128
BLOCK_M = 256
...
class HsacoPingpongFmha:
    """Loaded ping-pong code object with a reusable HIP launch buffer."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="correctness vs FlyDSL ping-pong only; skip the timed A/B",
    )
    parser.add_argument(
        "--gate-asm-cosine",
        action="store_true",
        help="accept cosine < 1e-4 vs production ASM when the clang roundtrip "
        "is not byte-identical to FlyDSL JIT",
    )
    ...
    module = HsacoPingpongFmha(args.co)
    ...
    if args.reference_co is not None:
        ...
        reference_module = HsacoPingpongFmha(args.reference_co)
    ...
    asm_o_ref, _ = production_asm()
    torch.cuda.synchronize()
    asm_cos = _cos_diff(custom_o, asm_o_ref)
    print(f"candidate vs production ASM cos_diff={asm_cos:.3e}")
    gate_ok = output_equal and lse_equal
    if not gate_ok and args.gate_asm_cosine and asm_cos < 1e-4:
        print("accepting clang roundtrip via production-ASM cosine gate")
        gate_ok = True
    if not gate_ok:
        module.close()
