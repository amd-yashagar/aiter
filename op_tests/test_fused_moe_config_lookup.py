...
def _config(block_m, label):
    return {
        "block_m": block_m,
        "ksplit": 0,
        "kernelName1": f"flydsl_{label}",
        "kernelName2": f"flydsl_{label}_s2",
        "run_1stage": False,
        "xbf16": 0,
        "flat": 0,
    }
...
def test_standard_ep_prefers_actual_topk_width():
    ...
    assert metadata.stage1.keywords["kernelName"] == "flydsl_standard_topk4"
...
def test_explicit_fake_slot_selects_routed_topk_width():
    ...
    assert metadata.block_m == 64
    assert metadata.stage1.keywords["kernelName"] == "flydsl_fake_slot_routed_topk4"
...
def test_legacy_fake_slot_fallback_remains_available():
    ...
    assert metadata.block_m == 64
    assert metadata.stage1.keywords["kernelName"] == "flydsl_legacy_fake_slot"
...
def test_public_api_exposes_explicit_fake_slot_keyword():
    assert "has_fake_topk_slot" in inspect.signature(fused_moe_api).parameters
    assert (
        "has_fake_topk_slot"
        in inspect.signature(fused_moe_module.fused_moe_fake).parameters
    )
    try:
        ...
        pytest.skip("aiter::fused_moe_ is not registered in this build")
    assert "has_fake_topk_slot" in str(schema)
